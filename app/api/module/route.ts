...
export const ModuleSchema = z.object({
  ...
  createdById: z.string()
    .min(1, { message: "El createdById es obligatorio y no puede estar vacío" })
});
...
export async function GET(req:NextRequest) {
    try {
      ...
      prisma.$disconnect()
       ...
       return  NextResponse.json(message({status:200, message:"modulos encontrados",data:{modules}}))
    } catch (error) {
        return NextResponse.json(message({status:400,message:"error General", data:error}))
    }
}
...
export async function POST(req:Request){
    try {
        ...
        const prisma = new PrismaClient()
        const {title, description,order,isActive, createdById} = validationBody.data
        ...
        return NextResponse.json(message({status:200,message:"Modulo creado", data:{moduleData:newModule}}))
    } catch (error) {
        return NextResponse.json(message({status:400,message:"Error en endpoint de creación de modulo", error, data:{error}}))
    }
}
